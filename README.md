...
2. Build the release packages: `make -f build/Makefile release`.  The files produced during the build should not be committed to the repository either. Note: On MacOSX, you may need to use the command `ulimit -n 1000` to complete the release. Also, there is now a `make -f build/Makefile release-notest` for people who really no what they are doing and don't want to run the tests!
