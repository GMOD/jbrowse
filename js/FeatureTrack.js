...
function FeatureTrack(config, refSeq, browserParams) {
	...
	Track.call(this, config.label, config.key, false, browserParams.changeCallback);
	this.fields = {};
	this.refSeq = refSeq;
	...
	// constructed here.
	var storeclass = config.backendVersion == 0 ? SeqFeatureStore.NCList_v0
			: SeqFeatureStore.NCList;
...
/**
 * Request that the track load its data. The track will call its own loadSuccess() function when it
 * is loaded.
 */
...
FeatureTrack.prototype.loadSuccess = function(trackInfo, url) {
	// AP: this needs is for demo purposes only! remove prior release
	var defaultConfig = {
		style : {
			className : "feature2",
		},
		...
		hooks : {
			// AP Can still left click and execute go to URL
	...
	if (!this.config.style.linkTemplate) {
		defaultConfig.events.click = function(track, elem, feat, event) {
			alert("clicked on feature\n" + "start: " + (Number(feat.get('start')) + 1) + ", end: "
					+ Number(feat.get('end')) + ", strand: " + feat.get('strand') + ", label: "
					+ feat.get('name') + ", ID: " + feat.get('id'));
		};
	}
	...
	for ( var event in this.config.events) {
		this.eventHandlers[event] = this.wrapHandler(this.evalHook(this.config.events[event]));
	}
...
FeatureTrack.prototype.evalHook = function(hook) {
	...
	try {
		...
		console.log("eval failed for hook on track " + this.name + ": " + hook);
	}
	return result;
};
...
FeatureTrack.prototype.wrapHandler = function(handler) {
	...
	return function(event) {
		...
		if (!elem.feature)
			return; // shouldn't happen; just bail if it does
		handler(track, elem, elem.feature, event);
	};
};

FeatureTrack.prototype.setViewInfo = function(genomeView, numBlocks, trackDiv, labelDiv, widthPct,
		widthPx, scale) {
...
/**
 * Return an object with some statistics about the histograms we will draw for a given block size in
 * base pairs.
 * 
 * @private
 */
FeatureTrack.prototype._histDimensions = function(blockSizeBp) {
	...
	for ( var i = 0; i < stats.length; i++) {
		if (stats[i].basesPerBin >= bpPerBin) {
			...
			pxPerCount = 100 / (logScale ? Math.log(stats[i].max) : stats[i].max);
			statEntry = stats[i];
			break;
		}
	}
...
FeatureTrack.prototype.fillHist = function(blockIndex, block, leftBase, rightBase, stripeWidth) {
	...
	var makeHistBlock = function(hist) {
		...
		for ( var bin = 0; bin < track.numBins; bin++) {
			...
			;
			binDiv.style.cssText = "left: " + ((bin / track.numBins) * 100) + "%; " + "height: "
					+ (dims.pxPerCount * (dims.logScale ? Math.log(hist[bin]) : hist[bin])) + "px;"
					+ "bottom: " + track.trackPadding + "px;" + "width: "
					+ (((1 / track.numBins) * 100) - (100 / stripeWidth)) + "%;"
	...
	if ((binCount > .9) && (Math.abs(binCount - Math.round(binCount)) < .0001)) {
		...
		histogramMeta.lazyArray.range(firstServerBin, firstServerBin + (binCount * this.numBins),
				function(i, val) {
		...
		// make our own counts
		this.featureStore.histogram(leftBase, rightBase, this.numBins, makeHistBlock);
	}
};
						...
						dojo.query('.feature', block)
								.forEach(
										function(featDiv) {
											...
											// get the feature start and end in terms of block width
											// pct
											var minLeft = parseInt(feature.get('start'));
											minLeft = 100 * (minLeft - block.startBase)
													/ blockWidth;
											var maxLeft = parseInt(feature.get('end'));
											maxLeft = 100 * ((maxLeft - block.startBase)
													/ blockWidth - labelDiv.offsetWidth
													/ block.offsetWidth);

											// move our label div to the view start if the start is
											// between the feature start and end
											labelDiv.style.left = Math.max(minLeft, Math.min(
													viewLeft, maxLeft))
													+ '%';
...
FeatureTrack.prototype.fillBlock = function(blockIndex, block, leftBlock, rightBlock, leftBase,
		rightBase, scale, stripeWidth, containerStart, containerEnd) {
	...
	if (this._updatedLabelForBlockSize != blockBases) {
		if (scale < (this.featureStore.density * this.config.scaleThresh.hist)) {
			this.setLabel(this.key + "<br>per "
					+ Util.addCommas(Math.round(blockBases / this.numBins)) + " bp");
		} else {
			this.setLabel(this.key);
		}
	...
	if (this.featureStore.histograms
		...
		this.fillHist(blockIndex, block, leftBase, rightBase, stripeWidth, containerStart,
				containerEnd);
	} else {
		...
		this.fillFeatures(blockIndex, block, leftBlock, rightBlock, leftBase, rightBase, scale,
				containerStart, containerEnd);
	}
};

/**
 * Creates a Y-axis scale for the feature histogram. Must be run after the histogram bars are drawn,
 * because it sometimes must use the track height to calculate the max value if there are no
 * explicit histogram stats.
 * 
 * @param {Number}
 *            blockSizeBp the size of the blocks in base pairs. Necessary for calculating histogram
 *            stats.
 */
FeatureTrack.prototype.makeHistogramYScale = function(blockSizeBp) {
	...
	if (this.yscale && this.yscale_params && this.yscale_params.maxval == maxval
...
/**
 * Called when sourceBlock gets deleted. Any child features of sourceBlock that extend onto
 * destBlock should get moved onto destBlock.
 */
FeatureTrack.prototype.transfer = function(sourceBlock, destBlock, scale, containerStart,
		containerEnd) {

	if (!(sourceBlock && destBlock))
		return;
	if (!sourceBlock.featureLayout)
		return;
	...
	var overlaps = (sourceBlock.startBase < destBlock.startBase) ? sourceBlock.featureLayout.rightOverlaps
	...
	for ( var i = 0; i < overlaps.length; i++) {
		...
		if (sourceSlot && sourceSlot.feature) {
			if (sourceSlot.layoutEnd > destLeft && sourceSlot.feature.get('start') < destRight) {
				...
				var featDiv = this.renderFeature(sourceSlot.feature, overlaps[i].id, destBlock,
						scale, containerStart, containerEnd);
...
FeatureTrack.prototype.fillFeatures = function(blockIndex, block, leftBlock, rightBlock, leftBase,
		rightBase, scale, containerStart, containerEnd) {
	// arguments:
	...
	var featCallback = function(feature, path) {
		...
		var featDiv = curTrack.renderFeature(feature, uniqueId, block, scale, containerStart,
				containerEnd);
	...
	this.featureStore.iterate(startBase, endBase, featCallback, function() {
		block.style.backgroundColor = "";
		curTrack.heightUpdate(layouter.totalHeight, blockIndex);
	});
};
...
FeatureTrack.prototype.measureStyles = function() {
	...
	heightTest.className = this.config.style.className + " plus-" + this.config.style.className
...
FeatureTrack.prototype.renderFeature = function(feature, uniqueId, block, scale, containerStart,
		containerEnd) {
	...
	if (name && (scale > this.labelScale)) {
		featureEnd = Math.max(featureEnd, featureStart
				+ ((name ? name.length : 0) * (this.nameWidth / scale)));
		levelHeight += this.nameHeight;
	}
	featureEnd += Math.max(1, this.padding / scale);

	var top = block.featureLayout.addRect(uniqueId, featureStart, featureEnd, levelHeight);
	...
	case 1:
	case '+':
		featDiv.className = featDiv.className + " plus-" + this.config.style.className;
		break;
	case -1:
	case '-':
		featDiv.className = featDiv.className + " minus-" + this.config.style.className;
		break;
	default:
		featDiv.className = featDiv.className + " " + this.config.style.className;
		break;
	}
	...
	featDiv.style.cssText = "left:" + (100 * (displayStart - block.startBase) / blockWidth) + "%;"
			+ "top:" + top + "px;" + " width:" + featwidth + "%;"
	...
	if (featwidth > minFeatWidth) {
		...
		if (subfeatures) {
			for ( var i = 0; i < subfeatures.length; i++) {
				this.renderSubfeature(feature, featDiv, subfeatures[i], displayStart, displayEnd);
			}
		}
	}
	...
	if (Util.is_ie6)
		featDiv.appendChild(document.createComment());
	// TODO: handle event-handler-related IE leaks
                
                
        /* Temi / AP adding right menu click
	// AP new schema menuTemplate: an array where everything except children, popup and url are
	// passed on as properties to a new dijit.Menu object
...
FeatureTrack.prototype.template = function(feature, temp ) {
  var urlValid = true;
  if ( temp ) {
...
FeatureTrack.prototype.renderSubfeature = function(feature, featDiv, subfeature, displayStart,
		displayEnd) {
	...
	if (this.config.style.subfeatureClasses) {
		...
		subDiv.className = this.config.style.subfeatureClasses[type] || this.config.style.className
				+ '-' + type;
		switch (subfeature.get('strand')) {
		case 1:
		case '+':
			subDiv.className += " plus-" + subDiv.className;
			break;
		case -1:
		case '-':
			subDiv.className += " minus-" + subDiv.className;
			break;
		}
	}
	...
	subDiv.style.cssText = "left: " + (100 * ((subStart - displayStart) / featLength)) + "%;"
			+ "top: 0px;" + "width: " + (100 * ((subEnd - subStart) / featLength)) + "%;";
 ...
 * 
 * This package and its accompanying libraries are free software; you can redistribute it and/or
 * modify it under the terms of the LGPL (either version 2.1, or at your option, any later version)
