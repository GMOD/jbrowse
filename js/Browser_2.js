...
//messing up starts here

    var recFoo = function(node) {
	var tt = [];
	var nom;
	for (var i = 2; i < node.length; i++) {
	    nom = brwsr.names.edgesFromNode(node[i], [])
	    tt = tt.concat(nom);	
	}
	return tt;
    };

    featureSelect = document.createElement("select");
    featureSelect.id = "featureSelect";
    ...
    var featureList;
    dojo.connect(testGo, "click", function(event) {
	featureList = recFoo(brwsr.names.root);	
//	alert(featureList); //gives out the array of feature names
	for (var i = 0; i < featureList.length; i++){
  	    addOption(featureSelect, featureList[i], featureList[i]);
	}
        dojo.stopEvent(event);
    });

    testGo.appendChild(document.createTextNode("feature list"));
    navbox.appendChild(testGo);
    navbox.appendChild(featureSelect);

    dojo.connect(featureSelect, "onchange", function(event) {
        brwsr.navigateTo( featureSelect.options[featureSelect.selectedIndex].value );
    });

    var testnode = ["ab", ,["e", "baz"],["foo", 1]];
    console.log(featureSelect.id); //featureSelect
    console.log(brwsr.names.edgesFromNode(testnode, [])); // LazyTrie
    console.log(brwsr.names.edgesFromNode(brwsr.names, [])); // []
    console.log(brwsr.names); // undefined
