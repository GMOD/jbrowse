    ...
    return declare(FeatureFiltererMixin, {
                        ...
                        thisB.loadUserCSS().then(function () {
                            ...
                            thisB.loadRefSeqs().then(function () {
                                ...
                                thisB.initView().then(function () {
                                    ...
                                    if (thisB.config.forceTracks)   { tracksToShow = tracksToShow.concat(thisB.config.forceTracks.split(',')); } else if (thisB.cookie('tracks')) { tracksToShow = tracksToShow.concat(thisB.cookie('tracks').split(',')); } else if (thisB.config.defaultTracks) {
        ...
        initView: function () {
            ...
            return this._milestoneFunction('initView', function (deferred) {
                ...
                } else if (this.config.show_fullviewlink) {menuBar.appendChild(this.makeFullViewLink());}
        ...
        saveSessionDir: function (directory) {
            ...
            if (array.every(obj, function (elt) { return elt.session !== dir; })) {obj.push({ session: dir });}
            ...
            return this._milestoneFunction('loadConfig', function (deferred) {
                ...
                if (this.config.dataRoot && this.config.dataRoot !== 'data' && !this.config.allowCrossOriginDataRoot) {
                    ...
                    if (parsedDataRoot.host) {
                        ...
                        if (parsedDataRoot.host !== currentParsed.host || parsedDataRoot.protocol !== currentParsed.protocol) {throw new Error('Invalid JBrowse dataRoot setting. For security, absolute URLs are not allowed. Set `allowCrossOriginDataRoot` to true to disable this security check.');}
                    }
                }
