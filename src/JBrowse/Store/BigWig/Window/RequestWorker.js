        ...
        if (this.blocksToFetch.length == 0) {
            ...
            var maybeCreateFeature = function(fmin, fmax, opts) {
                if (fmin <= this.max && fmax >= this.min) {
                    createFeature.call(this,fmin, fmax, opts);
                }
            };
            var tramp = function() {
                if (this.blocksToFetch.length == 0) {
                        ...
                        if (this.window.isSummary) {
                            ...
                            for (var i = 0; i < itemCount; ++i) {
                                ...
                                if (chromId == this.chr) {
                                    ...
                                    maybeCreateFeature.call( this, start, end, summaryOpts);
                                }
                            }
                            ...
                            if (blockType == this.BIG_WIG_TYPE_FSTEP) {
                                for (var i = 0; i < itemCount; ++i) {
                                    var score = fa[i + 6];
                                    maybeCreateFeature.call(this,blockStart + (i*itemStep), blockStart + (i*itemStep) + itemSpan, {score: score});
                                }
                            } else if (blockType == this.BIG_WIG_TYPE_VSTEP) {
                                for (var i = 0; i < itemCount; ++i) {
                                    var start = la[(i*2) + 6];
                                    var score = fa[(i*2) + 7];
                                    maybeCreateFeature.call(this,start, start + itemSpan, {score: score});
                                }
                                ...
                                for (var i = 0; i < itemCount; ++i) {
                                    ...
                                    maybeCreateFeature.call(this,start, end, {score: score});
                                }
                            } else {
                            ...
                            while (offset < ba.length) {
                                ...
                                if (bedColumns.length < 9) {
                                    if (chromId == this.chr) {
                                        maybeCreateFeature.call(this,start + 1, end, featureOpts);
                                    }
                                    ...
                                    for (var s = 0; s < tsList.length; ++s) {
                                        var ts = tsList[s];
                                        createFeature.call(this, ts.min() + 1, ts.max(), featureOpts);
                                    }
