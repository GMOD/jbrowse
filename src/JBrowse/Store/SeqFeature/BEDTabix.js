    ...
    return declare([ SeqFeatureStore, DeferredStatsMixin, DeferredFeaturesMixin, IndexedStatsEstimationMixin, RegionStatsMixin ], {
        ...
        getHeader: function () {
            if (!this._parsedHeader) {
                this._parsedHeader = this.indexedData.lineCount('nonexistent')
                ...
                this.indexedData.getMetadata()
                    .then(metadata => {
                        ...
                        return this.indexedData.getLines(
                            ...
                            query.end,
                            line => {
                                this.applyFeatureTransforms([this.lineToFeature(metadata.columnNumbers, line)])
                                    .forEach(f => {
                                        if (this.config.featureCallback) {f = this.config.featureCallback(f);}
                                        featureCallback(f);
                                    });
                            }
                        );
