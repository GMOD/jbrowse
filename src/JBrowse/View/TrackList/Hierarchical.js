    ...
    return declare(
        ...
        [ContentPane, _TextFilterMixin],
        {
            ...
            induceCategoryOrder(tracks, categoryOrder) {
                ...
                var unordered = tracks.filter(
                    (t) => order.indexOf(t.cat) === -1
                );
            ...
            startup() {
                ...
                for (var i = 0; i < arr.length; i++) {
                    if (arr[i].endsWith("*")) {
                        console.log({ starcollapse: arr[i] });
                        lang.setObject(
                            "starcollapse." + arr.slice(0, i + 1).join("/"),
                            true,
                            this.state
                        );
                    } else {
                ...
                this.get("trackMetaData").fetch({
                    ...
                    onComplete: () => {
                        ...
                        if (
                            !this.categories.Uncategorized.pane.containerNode
                                .children.length
                        ) {
                            this.categories.Uncategorized.pane.domNode.style.display =
                                "none";
                        }
                    },
                    sort: sorter,
            ...
            addTracks: function (tracks, inStartup) {
                ...
                array.forEach(
                    ...
                    function (track) {
                        ...
                        var categoryNames = (
                            (trackConf.metadata &&
                                trackConf.metadata[categoryFacet]) ||
                            trackConf[categoryFacet] ||
                            track[categoryFacet] ||
                            "Uncategorized"
                        ).split(/\s*\/\s*/);

                        let lastCat = categoryNames[0];
                        ...
                        if (typeof thisB.extendCheckbox === "function")
                            var checkBoxProps = thisB.extendCheckbox(
                                checkBoxProps,
                                trackConf
                            );
            ...
            setTracksActive: function (/**Array[Object]*/ trackConfigs) {
                array.forEach(
                    trackConfigs,
                    function (conf) {
                        this._findTrack(conf.label, function (
                            trackRecord,
                            category
                        ) {
