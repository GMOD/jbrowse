/* 
 * Extended from HTMLFeatures - for components with subfeatures, draw introns and improve box drawings.
 * Author: EY
 */
define( [
            'dojo/_base/declare',
    ...
    constructor: function( args ) {
        ...
        dojo.subscribe("/jbrowse/v1/n/tracks/redraw", function(data){
            require(["dojo/dom-construct", "dojo/_base/window"], function(domConstruct, win){
                ...
                query(".feature").forEach(function(node, index, arr){
                ...
                var featDiv = childNodes[i];
                        ...
                        if( strand < 0 && fmax > viewmin ) {
                            ...
                            for( var j = 0; j<featDivChildren.length; j++ ) {
                                ...
                                if(dojo.hasClass(arrowhead, "jb-intron")) {
...
//            featDiv.className = featDiv.className + " " + featDiv.className + "_phase" + phase;
    ...
    handleSubFeatures: function( feature, featDiv,
        ...
        this.renderIntrons(feature,featDiv,displayStart,displayEnd,block );
    },

    renderIntrons: function (feature, featDiv, displayStart, displayEnd, block )  {
        console.log("renderIntrons");
        console.log(feature.get('type')+": "+feature.get('start')+',' + feature.get('end') +',' + feature.get('strand'));
        console.dir(feature);
        console.dir(block);
        ...
        if (subfeatures) {
            subfeatures.sort(function(a, b){ return a.get('start') - b.get('start'); });    

            for (var i = 0; i< subfeatures.length; i++) {
                var feature = subfeatures[i];
                console.log(feature.get("type")+": "+feature.get("id")+","+feature.get('start')+','+feature.get('end')+','+feature.get('strand'));
            }
            
            require(["dojo/dom-construct", "dojo/_base/window"], function(domConstruct, win){

                for(var i=0; i< subfeatures.length-1;++i) {
                    var gap = subfeatures[i+1].get("start")-subfeatures[i].get("end");
                    if (gap > 2) {
                        //console.log("gap of "+gap+" between "+i+" and "+(i+1));
                        // render intron

                        var a_left  = subfeatures[i].get('start');
                        var a_width = subfeatures[i].get('end') - a_left;

                        var b_left  = subfeatures[i+1].get('start');
                        var b_width = subfeatures[i+1].get('end') - b_left;

                        var subStart = a_left+a_width;
                        var subEnd = b_left;
                        
                        var pxWidth = (100 * ((subEnd - subStart) / featLength));
                        /*
                        var subDiv = domConstruct.create("svg", {
                            innerHTML: "<polyline points='0,5 5,0 10,5' style='fill:none;stroke:black;stroke-width:1' />",
                            viewBox: "0 0 10 10",
                        ...
                        */
                        //domConstruct.place("<polyline points='0,5 5,0 10,5' style='fill:none;stroke:black;stroke-width:1' />", subDiv);
                        var left = (100 * ((subStart - displayStart) / featLength))+"%";
                        var width = (100 * ((subEnd - subStart) / featLength)) + "%";
                        var height = "11px";
                        
                        var str = "<svg class='jb-intron' viewBox='0 0 100 100' preserveAspectRatio='none' ";
                        str += "style='position:absolute;";
                        str += "left:"+left+";width:"+width+";height:"+height+"'>";
                        str += "<polyline points='0,50 50,5 100,50' style='fill:none;stroke:black;stroke-width:3' />";
                        str += "</svg>";
                        //featDiv.innerHTML = featDiv.innerHTML + str;
                        
                        var subDiv = domConstruct.place(str, featDiv);
                        
                        //block.featureNodes[ subfeature[i].id() ] = subDiv;
    ...
    renderSubfeature: function( feature, featDiv, subfeature, displayStart, displayEnd, block ) {
        ...
        console.log("subfeatureid = "+subfeature.id());
