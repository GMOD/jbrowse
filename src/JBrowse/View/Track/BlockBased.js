 ...
 */
     ...
     */
            ...
            showRange: function (first, last, startBase, bpPerBlock, scale,
                ...
                var firstAttached = (this.firstAttached == null ? last + 1 : this.firstAttached);
                var lastAttached =  (this.lastAttached == null ? first - 1 : this.lastAttached);
     ...
     */
            ...
            hideAll: function () {
                if (this.firstAttached == null) return;
                for (var i = this.firstAttached; i <= this.lastAttached; i++) {this._hideBlock(i);}
     ...
     */
            ...
            _handleError: function (error, viewArgs) {
                ...
                if (isObject && error instanceof Errors.TimeOut && errorContext.block) {this.fillBlockTimeout(errorContext.blockIndex, errorContext.block, error);} else if (isObject && error instanceof Errors.DataOverflow) {
                    if (errorContext.block) {this.fillTooManyFeaturesMessage(errorContext.blockIndex, errorContext.block, viewArgs.scale, error);} else {
            ...
            _processMenuSpec: function (spec, context) {
                for (var x in spec) {
                    if (spec.hasOwnProperty(x)) {
                        if (typeof spec[x] === 'object') {spec[x] = this._processMenuSpec(spec[x], context);} else {spec[x] = this.template(context.feature, this._evalConf(context, spec[x], x));}
                    }
                }
                return spec;
            },
     ...
     */
            ...
            _openDialog: function (spec, evt, context) {
                ...
                function setContent(dialog, content) {
                    ...
                    if (typeof content.then === 'function') {content.then(function (c) { dialog.set('content', c); });}
