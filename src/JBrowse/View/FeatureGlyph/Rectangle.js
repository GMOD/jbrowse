...
return declare( FeatureGlyph, {
    ...
    renderFeature: function( context, viewArgs, fRect ) {
        var width = Math.max( fRect.w, 1 );
        // background
        var color = this.getStyle( fRect.f, 'color' );
        if( color ) {
            context.fillStyle = color;
            context.fillRect( fRect.l, fRect.t, width, fRect.h );
        }
        ...
        if( fRect.h > 3 ) {
            ...
            if( border_color ) {
                ...
                context.strokeRect( fRect.l+0.5, fRect.t+0.5, width-1, fRect.h-1 );
            }
        }
        else if( fRect.h > 1 ) {
            border_color = this.getStyle( fRect.f, 'border_color' );
            if( border_color ) {
                ...
                context.fillRect( fRect.l, fRect.t+fRect.h-1, width, 1 );
            }
        }
    }
