...
function (
    ...
    return declare(null, {
        constructor: function (args, parent) {
            ...
            var columns = [];
            if (array.some(locations, function (l) { return l.label; })) {columns.unshift({ label: 'Name', field: 'label' });}
            if (array.some(locations, function (l) { return l.description; })) {columns.unshift({ label: 'Description', field: 'description' });}
            if (array.some(locations, function (l) { return l.score; })) {columns.unshift({ label: 'Score', field: 'score' });}
