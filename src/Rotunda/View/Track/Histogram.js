...
return declare (Track,
{
    ...
    buildHistogramForView: function (rot, callback, errorCallback) {
	return getFeaturesInView (rot, callback, errorCallback)
    },

    draw: function (rot, minRadius, maxRadius) {
	var track = this
	track.buildHistogramForView (rot, function (features) {
            var values = features.map (function (feature) { return feature.value })
		...
		.innerRadius (function(feature) {
            ...
            var path = this.d3featData (rot, features)
		.append("path")
		.attr("d", featureArc)
